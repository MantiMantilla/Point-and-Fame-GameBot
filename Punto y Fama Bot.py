...
"""whether digit j in J was guessed for the person p in P for trial i in II_p\
on position n in N"""
...
"""Independent of their position, the digits guessed for person p in p in\
...
for p in P:
    for i in II[p]:
        ...
        prob += (
            lp.lpSum(
                lp.lpSum(x[(p, n, j)] for n in N)
                * lp.lpSum(dig[(p, i, n, j)] for n in N)
                for j in J
            )
            == pun[(p, i)] + fam[(p, i)]
        )
...
for p in P:
    for i in II[p]:
        print(f"retricc. 2: {i}")
        prob += (
            lp.lpSum(dig[(p, i, n, j)] * x[(p, n, j)] for n in N for j in J)
            == fam[(p, i)]
        )
...
while loop_condition:
    ...
    for p in P:
        print(f"Player {p}:")
        next_guess = "".join(
            [str(int(round(sum(j * x[p, n, j].value() for j in J)))) for n in N]
        )
        print(f"Is your number {next_guess}?")
    ...
    for p in P:
        ...
        new_points = int(
            input(f"How many points did this guess earn? (1 - {len(N)}): ")
        )
        ...
        for n in N:
            for j in J:
                if int(new_guess[int(n) - 1]) == j:
                    ...
                    dig[(person, i, n, j)] = 0
        """Independent of their position, the digits guessed for person p in p\
        in round i in II_p must match their chosen number by as many positions\
        as the guess has points and fames"""
        prob += (
            lp.lpSum(
                lp.lpSum(x[(person, n, j)] for n in N)
                * lp.lpSum(dig[(person, i, n, j)] for n in N)
                for j in J
            )
            == pun[(person, i)] + fam[(person, i)]
        )

        """The digits chosen by person p in P match the digits guessed on\
        trial p in P"""
        prob += (
            lp.lpSum(dig[(person, i, n, j)] * x[(person, n, j)] for n in N for j in J)
            == fam[(person, i)]
        )
